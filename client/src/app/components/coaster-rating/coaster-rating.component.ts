...
export class CoasterRatingComponent {
  displayAvgRating: string;
  stars: any[];
  halfStars: any[];
  emptyStars: any[];

  private _ratingCounts: number[];

  get ratingCounts() {
    return this._ratingCounts;
  }

  @Input()
  set ratingCounts(value: number[]) {
    this._ratingCounts = value;
    
    if (this._ratingCounts) {
      let ratingSum = 0;
      let totalCount = 0;
